...
// ── Mapa: valor guardado en BD → clave de traducción ──────────────────────────
const ROLE_KEY_MAP: Record<string, string> = {
  security_analyst:  'pages.teamPage.modal.roles.securityAnalyst',
  pentester:         'pages.teamPage.modal.roles.pentester',
  security_engineer: 'pages.teamPage.modal.roles.securityEngineer',
  read_only:         'pages.teamPage.modal.roles.readOnly',
};
...
export default function MemberCard({ member, onEdit, onDelete }: MemberCardProps) {
  ...
  // Traduce el rol si está en el mapa, si no muestra el valor crudo de BD
  const roleLabel = ROLE_KEY_MAP[member.role]
    ? t(ROLE_KEY_MAP[member.role])
    : member.role;
    ...
    <article className="bg-bg-secondary border border-border-primary rounded-xl p-6 mb-4 transition-colors hover:border-border-secondary">
      ...
      <div className="flex items-start gap-4 mb-5">
        ...
        <div className="flex-1 min-w-0">
          ...
          {/* ← CAMBIO: roleLabel en lugar de member.role */}
      ...
      <div className="bg-bg-tertiary border border-border-primary rounded-xl p-4 mb-4">
        ...
        <p className="text-sm text-text-secondary mt-3">
          {t('pages.teamPage.member.activeTasks', { count: member.active_tasks_count ?? 0 })}
        </p>
